...
`;
            ...
            chunk.forEach(q => {
                ...
                // INLINE CODE REPLACEMENT LOGIC FOR PDF
                ...
                const processOpt = (opt, isCode) => {
                    ...
                    if (isCode) {
                        // Safely escape everything for a \texttt{} LaTeX block, replacing spaces with ~
                        let codeSafe = opt
                ...
                if (hasOptions) {
                    // Check if we should use 2 columns based on Editor's layout toggle or Auto-detect
                    const isTwoCol = q.optLayout === '2col' || (q.optLayout !== '1col' && shouldUseTwoCols(q));

                    if (isTwoCol) {
                        // Use a LaTeX Tabular environment for a perfect 2x2 grid
                        latex += "\n\\vspace{0.3cm}\\noindent\\begin{tabular}{@{}p{0.48\\linewidth} p{0.48\\linewidth}@{}}\n";
                        
                        let row1 = "";
                        // Notice the q.isCodeOptions added here!
                        ...
                        // Fallback to standard 1-column list
                        latex += "\\begin{choices}\n";
