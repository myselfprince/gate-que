...
export default function ScraperPage() {
  ...
  useEffect(() => {
    queueMicrotask(loadAvailableBackups);
    // Browser storage only needs to be scanned once on mount.
  ...
  const handleCopyRange = (questionsArray, chapterName, rangeStr) => {
    ...
    // NEW: Apply the filter to the sliced range!
  ...
  // NEW: Get all unique years across all scraped questions
      ...
      {Object.keys(groupedQuestions).length > 0 && (
        <div style={{ background: '#313244', padding: '15px', borderRadius: '10px', display: 'flex', gap: '15px', alignItems: 'center', marginBottom: '20px', border: '1px solid #a6e3a1', flexWrap: 'wrap', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flex: '1 1 auto' }}>
            ...
            {/* NEW: Dropdown to filter by year */}
            <div>
      ...
      <div id="questions_render_container">
        <div style={{ display: 'grid', gap: '15px' }}>
          {Object.entries(groupedQuestions).map(([chapterName, questionsArray]) => {
            // Check visibility dynamically
            const visibleQs = filterYear === 'All' ? questionsArray : questionsArray.filter(q => q.year === filterYear);
            
            // Auto-hide the entire chapter container if filter returns 0 matches
                    ...
                    {questionsArray.map((q, idx) => {
                      ...
                      return (
                        <div key={q.id} style={{ background: '#1e1e2e', padding: '20px', borderRadius: '8px', border: '1px solid #45475a' }}>
                          ...
                          <div style={{ marginBottom: '15px', whiteSpace: 'pre-wrap' }}>{q.text}</div>
                          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                            {q.optA && <div style={{ background: '#11111b', padding: '10px', borderRadius: '4px', border: '1px solid #45475a', whiteSpace: 'pre-wrap' }}><strong>(A)</strong> {q.optA}</div>}
                            {q.optB && <div style={{ background: '#11111b', padding: '10px', borderRadius: '4px', border: '1px solid #45475a', whiteSpace: 'pre-wrap' }}><strong>(B)</strong> {q.optB}</div>}
                            {q.optC && <div style={{ background: '#11111b', padding: '10px', borderRadius: '4px', border: '1px solid #45475a', whiteSpace: 'pre-wrap' }}><strong>(C)</strong> {q.optC}</div>}
                            {q.optD && <div style={{ background: '#11111b', padding: '10px', borderRadius: '4px', border: '1px solid #45475a', whiteSpace: 'pre-wrap' }}><strong>(D)</strong> {q.optD}</div>}
                          </div>
